<x-app-layout>
        ...
        <form method="POST" action="{{ route('cotizaciones.update', $cotizacion->id) }}" class="max-w-xl bg-white p-6 rounded-lg shadow-md">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="mb-4">
                    ...
                    <input type="date" name="fecha_cot" id="fecha_cot" value="{{ $cotizacion->fecha_cot->toDateString() }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    @error('fecha_cot')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="vigencia" id="vigencia" value="{{ $cotizacion->vigencia->toDateString() }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    @error('vigencia')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="mb-4">
                ...
                @foreach($productos as $producto)
                    <div class="producto-item flex items-center mb-2">
                        ...
                        @if($cotizacion->productos->contains($producto->id)) checked @endif>
                        <span class="ml-2 text-gray-700">{{ $producto->nombre }} - ${{ number_format($producto->PC, 2) }}</span>
                        <input type="number" name="cantidad[]" value="{{ $cotizacion->productos->where('id', $producto->id)->first()->pivot->cantidad ?? 1 }}" min="1" class="ml-2 w-16 px-2 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                @endforeach
                @error('products')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Agregar event listeners para los checkboxes y las entradas de cantidad
        ...
        // Llamar a la función al cargar la página para establecer los valores iniciales
